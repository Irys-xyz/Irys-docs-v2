...
const getWebIrysBundler => {
  try {
    const provider = new ethers.providers.Web3Provider(window.ethereum);
    const webBundlerIrys = await WebEthereumBundlerIrys().withProvider(Ethersv5Adapter(provider));
    console.log(`Connected from ${webBundlerIrys.address}`);

    return webBundlerIrys;
  } catch (error) {
...
const getWebIrysBundler = async (): Promise<void> => {
  ...
  const webIrysBundler = await WebEthereumBundlerIrys().withProvider(Viemv2Adapter(provider));
  console.log(`Connected from ${irys.webIrysBundler}`);
