...
In your `App.js` file write an initialization function that sets up a `WebIrys` object. The following code shows how to use ethers5, if you're using [a different provider](/build/d/irys-in-the-browser#supported-providers) make sure to specify it when creating the `wallet`.
