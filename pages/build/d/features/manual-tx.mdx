...
In addition to uploading using the SDK functions [`irys.upload()`](/build/d/sdk/api/upload),[`irys.uploadFile()`](/build/d/sdk/api/uploadFile),
and [`irys.uploadFolder()`](/build/d/sdk/api/uploadFolder), you can also manually create, sign, and upload a transaction in separate steps.
...
const createSignUpload = async () => {
	...
	const tx = irys.createTransaction("hirys!", { tags: [{ name: "Content-Type", value: "text/plain" }] });
	...
	console.log(`Tx uploaded. https://testnet-gateway.irys.xyz/${receipt.id}`);
};
```
...
const serializationUpload = async () => {
	...
	// Create the transaction
	const tx1 = irys.createTransaction("hirys!", { tags: [{ name: "Content-Type", value: "text/plain" }] });
