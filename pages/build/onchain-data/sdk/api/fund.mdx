...
- `amount`: The amount to fund [in atomic units](/build/onchain-data/sdk/api/utils-toAtomic) 
...
Upfront funding reduces the number of transactions required when uploading. You fund once and then when uploading, payment is deducted directly from your account. You can also [withdraw](/build/onchain-data/sdk/api/withdrawBalance) any excess balance if needed.
...
try {
	...
	console.log(`${pathToFile} --> Uploaded to https://testnet-gateway.irys.xyz/${id}`);
} catch (e) {
	console.log("Error funding node ", e);
}
```
