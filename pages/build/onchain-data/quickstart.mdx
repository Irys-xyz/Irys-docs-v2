...
Connect using a private key when using an EVM or Solana wallet:
...
Or using a serialized JWK file when using an Arweave wallet:
...
Fund a node using any of our [supported tokens](/build/onchain-data/features/supported-tokens):
...
Data uploaded to Irys is given a millisecond-accurate timestamp, attributes and authorship details before being being stored onchain. 

This information is used to create a signed receipt that be trustlessly [verified](/build/onchain-data/sdk/api/utils-verifyReceipt).
...
In addition to using the [query package](/build/onchain-data/querying/query-package) to search Irys, you can also search directly from the Irys SDK. The following shows how to search for all transactions posted to [Irys mainnet](/overview/networks), paid for with Matic, and uploaded during a three day period.
