...
Use the query package to search transactions posted to Irys. 
...
## Query location (mainnet, devnet)

The `Query` class defaults to querying Irys's mainnet. To search Irys's devnet, use the `network` parameter.
...
Use the `from()` function to search by wallet addresses used when signing and paying for the upload. Addresses from any of [Irys's supported chains](/overview/supported-tokens) are accepted.
...
Use the `to()` function to search for the wallet address of the transaction recipient. 

```js
const results = await myQuery.search("irys:transactions").to("TrnCnIGq1tx8TV8NA7L2ejJJmrywtwRfq9Q7yNV6g2A");
```

## Token

Use the `token()` function to search based on the token name used to pay for the upload. Any of [these values](/build/onchain-data/features/supported-tokens) are acceptable.
...
Use the `ids()` function to search for blocks with the specified IDs.
...
```js
const results = await myQuery.search("irys:blocks").minHeight(1188272).maxHeight(1188279);
```

Or for transactions within the specified block height range. Results are `>= minHeight and < maxHeight`.

```js
const results = await myQuery.search("irys:transactions").minHeight(1188272).maxHeight(1188279);
```

### Converting Timestamp to Block Height

Irys block time is approximately 2 seconds. Use this code to find the closest block height for a given timestamp.
...
Use the `limit()` function to limit the maximum number of results returned. This overrides the default value of 1000 results.
...
.fields({
	...
	receipt: {
		deadlineHeight: true, // The block number by which the transaction must be finalized
...
When searching by `irys:blocks` the following fields are available:
