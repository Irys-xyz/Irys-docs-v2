...
Use the `fromTimestamp()` and [`toTimestamp()`](./toTimestamp) functions to search for transactions by timestamp. Results returned are `>= fromTimestamp` and `< toTimestamp`.
