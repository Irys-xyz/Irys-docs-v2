...
Transaction metadata can be [queried](/build/onchain-data/querying/query-package) directly using the [Irys query package](/build/onchain-data/querying/query-package) a REST API or [GraphQL](/build/onchain-data/querying/graphql). This enables users to create tailored queries, such as searching for specific [tags](/build/onchain-data/features/tags) or addresses.

Irys supports adding custom metadata [tags](/build/onchain-data/features/tags) to each upload, these tags allow users to build data models, all queryable via GraphQL.
...
To download data from a gateway, use the transactionobtained during upload. The most commonly used gateway is `https://gateway.irys.xyz`, and the format used to download data is `https://gateway.irys.xyz/[transaction-id]`.
