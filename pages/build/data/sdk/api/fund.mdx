...
<ApiTable columns={[
	...
	{ header: "Description", key: "description", span: 8 },
	  ...
	  { name: "amount", type: "number", description: "The amount to fund [in atomic units](/build/data/sdk/api/utils-toAtomic)" },
	  { name: "multiplier", type: "number", description: "Fee multiplier (optional)" },
]}/>
...
Upfront funding reduces the number of transactions required when uploading. You fund once and then when uploading, payment is deducted directly from your account. You can also [withdraw](/build/data/sdk/api/withdrawBalance) any excess balance if needed.
...
try {
  ...
  console.log(
    `${pathToFile} --> Uploaded to https://testnet-gateway.irys.xyz/${id}`
  );
} catch (e) {
  console.log("Error funding node ", e);
}
