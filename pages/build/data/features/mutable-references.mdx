...
1. Upload a base transaction to Irys and reference it using a URL in the following format `https://testnet-gateway.irys.xyz/mutable/:id`

```js
const irys = await getIrys();
const receiptOne = await irys.upload("First TX");
console.log(`TX 1 uploaded https://testnet-gateway.irys.xyz/mutable/${receiptOne.id}`);
```
...
console.log(`TX 2 uploaded https://testnet-gateway.irys.xyz/mutable/${receiptOne.id}`);
```

The original URL (`https://testnet-gateway.irys.xyz/mutable/:id`) now resolves to the second transaction in the chain.
...
While the `https://testnet-gateway.irys.xyz/mutable/:id` endpoint will always resolve to the most recent transaction in a chain, it is possible to directly access any transaction in a chain using the transaction’s ID and a URL in the format `https://testnet-gateway.irys.xyz/:id`
