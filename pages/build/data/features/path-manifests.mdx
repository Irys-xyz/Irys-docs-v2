...
`https://testnet-gateway.irys.xyz/:manifestId/:pathName`
...
`https://testnet-gateway.irys.xyz/W1UbYAZ08egXgm9_kCw24ZZPAfdu8LQB7jc_Vx8fwv4/foo1.png`
...
1.  Uses the [Irys query package](/build/data/graphql) to search for the 10 most recent transactions tagged `image/png`.
...
const getTxIds = async (totalIds: number): Promise<string[]> => {
	const myQuery = new Query({ network: "testnet" });
...
const generateManifest = async () => {
	...
	map.forEach((value, key) => {
		console.log(`https://testnet-gateway.irys.xyz/${manifestId}/${key}`);
	});
};
