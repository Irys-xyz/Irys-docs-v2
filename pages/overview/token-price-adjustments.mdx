...
This process continues for each subsequent block, using the block's price and the previous block's EMA as part of the calculation. Upon reaching EMA9 (E9 in the diagram), the calculated EMA9 is recorded in the block for use at block height = 30.
...
For miners quoting a price to users for storage during the 3rd Adjustment interval, they would take whatever price was higher the E9 token price or the E19, so that even if a data storage transaction was posted in the 3rd Adjustment Interval it could still be confirmed in the 4th.
...
- **Median**: More stable than average price but requires sorting of all the prices in the sampling range. It also decreases the accuracy of the price prediction as it looks at the range as a whole leading to a wider margin of error.

- **EMA**: Is frequently used in financial applications. It smooths out price volatility while weighting recent prices with higher impact than older prices giving a nice blend of protection from volatility and accuracy.

### Why not use Arweave’s dynamic pricing?

Arweave’s dynamic pricing adjusts based on miner activity, specifically the rate at which miners join or leave the network. This behavior informs the storage price adjustments. However, the AR token price is highly volatile, and miners tend to be sticky, meaning they are not quick to abandon their investment. Onboarding to mine Arweave can take several weeks, a period that extends as Arweave’s dataset grows. Consequently, the network’s fee adjustments lag behind token price changes, resulting in volatility in the price per GB of storage, which can deter users.
