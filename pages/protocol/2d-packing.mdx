...
Packing is the process of adding a unique cryptographic fingerprint to stored data, and is used to prove to the network that miners are providing unique resources. This is essential to prevent adversarial participants from claiming to store multiple copies of data or offering large storage capacities while only providing a fraction of the claimed resources.

Irys introduces a novel approach to packing called 2D Packing which, combined with staking and slashing mechanisms, results in fast and efficient storage verification. Irys' innovative approach reduces the risk of computational exploits by adversarial miners.
...
- Ensures a consistent packing time on a single CPU core.
