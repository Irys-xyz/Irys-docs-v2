...
The simplest form of transaction on Irys is a value transfer, it transfers native $IRYS tokens from one wallet to another. These transactions are very similar to those you would see on Bitcoin or Ethereum and are a fundamental part of any blockchain and its economic incentives.

## Data Transactions

Irys stores data using data transactions. These transactions include properties about the data to be stored, and a single upfront payment covering the storage term.

### Permanent Data

Irys’ [Publish Ledger](/protocol/ledgers#permanent-data-publish-ledger) stores data permanently. Users pay for 10 replicas of their data stored for 200 years (with a wide safety margin).
