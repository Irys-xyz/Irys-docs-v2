---
description: Irys' partitions optimize data storage and replication, ensuring verifiable distribution, performance-based rewards, and adaptive scaling. This maintains network integrity and efficient pricing for users.
---
...
Partitions are the fundamental building blocks of Irys’ [data ledgers](./ledgers), enabling control and optimization of the network. This approach allows for:
...
<img className="rounded-2xl my-5" src="/diagrams/protocol/partition-ledger.jpg" />
...
- Multiple partitions reside at each index in the ledger, replicating the data and ensuring fault-tolerance.
...
By enabling miners to commit to specific partitions, the protocol organizes these [partitions](/protocol/partitions) into distinct [ledgers](/protocol/ledgers), offering unparalleled flexibility.

This approach allows for multiple data ledgers, each tailored to different storage needs. At launch, there will be two ledgers: a [Permanent Ledger](/protocol/ledgers#permanent-data-publish-ledger) for long-term, immutable storage and a [Temporary Ledger](/protocol/ledgers#term-data-ledgers) for short-term, flexible storage solutions.

## Capacity Partitions

<img className="rounded-2xl my-5" src="/diagrams/protocol/capacity-partitions.jpg" />

> Where do partitions come from, and how are they associated with a ledger?

Irys measures the amount of data uploaded and then uses it to predict the number of partitions needed in the immediate future. These standby partitions do not contain any data (yet) and are referred to as Capacity Partitions. Miners mine empty capacity partitions, enabling them earn rewards. The protocol keeps these Capacity Partitions on standby and uses them to add storage capacity to Irys' data ledgers based on realtime user demand.
...
The miner packs the Capacity Partition using [Irys’ 2D Packing](/architecture/2d-packing). This process fully encodes the miner’s fingerprint into the partition.
