...
Irys measures the amount of data uploaded and then uses it to project the number of partitions needed on standby. These standby partitions do not contain any data (yet) and are referred to as Capacity Partitions. Miners mine empty capacity partitions, enabling them earn rewards. The protocol keeps these Capacity Partitions on "standby" and uses them to add storage capacity to Irys' data ledgers based on realtime user demand.
